...
function 매출중복정리_(dryRun) {
  Object.keys(BRANCH_CONFIG).forEach(function (branch) {
    ...
    for (var i = 0; i < v.length; i++) {
      ...
      //
      //  그래서 「표식 달의 앞뒤 10일」까지는 정상으로 본다.
      //     표식 202606 · 날짜 05-31  →  6/1 과  1일 차이   정상
      //     표식 202606 · 날짜 05-28  →  6/1 과  4일 차이   정상 (몰아서 올린 것)
      //     표식 202606 · 날짜 05-13  →  6/1 과 19일 차이   오류
      //
      //  ⚠️ 왜 10일인가 — 숫자로 맞춰봤다.
      //     5/28 (1,237,000원) 을 지우면 5월이 포스보다 134만원 적어진다.
      //     배달매출이 마이너스가 되는 셈이라 말이 안 된다. 그러니 진짜 매출이다.
      //     3일이면 이게 걸리고, 10일이면 안 걸린다.
      //     ⚠️ 대신 백석 6/22 (624,000원, 9일 벗어남) 같은 것도 못 잡는다.
      //        그건 근거가 없어 판단을 미룬 것이다.
...
function 알바백업진단() {
  ...
  // ⚠️ 2026-09-07 — 첫 판에서 「사흘 넘으면 멈춘 것」이라고 했는데 틀렸다.
  //    알바데이터_백업() 은 내용이 바뀌었을 때만 쌓는다.
  //        if (prev === text) { 변경 없음 — 건너뜀 }
  //    급여 변동이 없으면 며칠이고 안 쌓이는 게 정상이다.
  //    실제로 8/18 → 9/2 도 보름이 비어 있었지만 정상이었다.
  if (며칠 >= 10) {
    Logger.log('  ⚠️ 열흘 넘게 안 쌓였습니다.');
    Logger.log('     급여 변동이 없었으면 정상입니다 (내용이 바뀔 때만 쌓습니다).');
    Logger.log('     그 사이 알바를 넣거나 급여를 지급하셨다면 확인이 필요합니다.');
  } else {
    Logger.log('  ✅ 정상입니다. (급여 변동이 없으면 며칠이고 안 쌓이는 게 맞습니다)');
  }
...
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  🧾 종소세수식 — 종합소득세 예수금 수식을 고친다  (원당점만)
//
//  2026-09-13 추가.
//
//  ── 무엇이 잘못돼 있었나 ──────────────────────────────────
//    지금      =C14 × D52     「현금매출」의 2%
//    현금은 매출의 3~4%뿐이라, 사실상 아무것도 안 잡히고 있었습니다.
//        8월 실제 잡힌 금액   54,280원
//        1년이면              약 65만원
//        수기 실제 납부액      9,139,160원   ← 실제의 7% 밖에 안 됨
//
//  ── 어떻게 바꾸나 ────────────────────────────────────────
//    바꾼 뒤   =C13 × D52     「매출액 전체」의 1.1%
//
//    1.1% 의 근거
//        시트 1~8월 매출 합   552,406,830원
//        연 환산              약 828,610,000원
//        수기 종합소득세        9,139,160원
//        9,139,160 ÷ 828,610,000 = 1.10%
//
//  ⚠️ 종합소득세는 원래 매출이 아니라 이익에 매깁니다. 이건 어림값입니다.
//     매년 실제 납부액을 보고 아래 새비율_ 을 다시 맞추세요.
//
//  ⚠️ 백석은 건드리지 않습니다. 백석 실제 납부액을 모릅니다.
//  ⚠️ 「26년 x월 손익계산서」 템플릿도 함께 고칩니다.
//     이걸 빠뜨리면 다음 달 새로 생기는 시트가 또 옛 수식을 갖습니다.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

var 새비율_      = 0.011;                 // 1.1%
var 종소세행이름_ = '종합소득세 예수금';
var 매출액행이름_ = '매출액';

function 종소세수식_미리보기() { 종소세수식_(true); }
function 종소세수식_적용()     { 종소세수식_(false); }

function 종소세수식_(dryRun) {
  var branch = '원당점';
  var ss = SpreadsheetApp.openById(BRANCH_CONFIG[branch].ssId);

  var 탭들 = ['26년 x월 손익계산서'];         // 템플릿 먼저
  for (var m = 1; m <= 12; m++) 탭들.push('26년 ' + m + '월 손익계산서');

  Logger.log('\n ════════ 원당 종합소득세 예수금 수식 고치기 ════════\n');
  Logger.log('  바꿀 내용   =C14×D  (현금매출 2%)  →  =C{매출액행}×D  (매출액 ' +
             (새비율_ * 100).toFixed(1) + '%)\n');
  ...
  탭들.forEach(function (tabName) {
    ...
    // 이름으로 행을 찾는다 — 행 번호가 달마다 달라도 안전하다
    var 종소세행 = -1, 매출액행 = -1;
    for (var i = 0; i < last; i++) {
      var name = String(b[i][0] || '').replace(/\s+/g, '');
      if (종소세행 < 0 && name === 종소세행이름_.replace(/\s+/g, '')) 종소세행 = i + 1;
      if (매출액행 < 0 && name === 매출액행이름_) 매출액행 = i + 1;
    }

    if (종소세행 < 0) { Logger.log('  ⏭ ' + tabName + ' — 「종합소득세 예수금」 줄 없음'); return; }
    if (매출액행 < 0) { Logger.log('  ⚠️ ' + tabName + ' — 「매출액」 줄을 못 찾음. 건너뜁니다'); 건너뜀++; return; }

    var 새수식  = '=C' + 매출액행 + '*D' + 종소세행;
    var 현재수식 = sh.getRange(종소세행, 3).getFormula();
    var 현재비율 = sh.getRange(종소세행, 4).getValue();
    var 현재값  = sh.getRange(종소세행, 3).getDisplayValue();
    var 매출액  = Number(String(sh.getRange(매출액행, 3).getDisplayValue()).replace(/[^0-9.-]/g, '')) || 0;
    var 새값    = Math.round(매출액 * 새비율_);

    var 수식같음 = 현재수식.replace(/\s+/g, '') === 새수식;
    var 비율같음 = Math.abs(Number(현재비율) - 새비율_) < 0.0001;

    if (수식같음 && 비율같음) { Logger.log('  ✅ ' + tabName + ' — 이미 되어 있음'); return; }

    Logger.log('  · ' + tabName + '   ' + 종소세행 + '행  (매출액은 ' + 매출액행 + '행)');
    Logger.log('      전:  ' + (현재수식 || '(수식 없음)') + '   비율 ' +
               (Number(현재비율) * 100).toFixed(2) + '%   →  ' + 현재값);
    Logger.log('      후:  ' + 새수식 + '   비율 ' +
               (새비율_ * 100).toFixed(1) + '%   →  ' + 새값.toLocaleString() + '원');
    바꿀것++;

    if (!dryRun) {
      sh.getRange(종소세행, 3).setFormula(새수식);
      sh.getRange(종소세행, 4).setValue(새비율_);
    }
  });

  Logger.log('\n  ──────────────────────────────');
  Logger.log('  바꿀 탭 ' + 바꿀것 + '개' + (건너뜀 ? ' · 건너뛴 탭 ' + 건너뜀 + '개' : ''));

  if (dryRun) {
    Logger.log('\n  ※ 미리보기입니다. 아무것도 안 바꿨습니다.');
    Logger.log('  ※ 맞으면 종소세수식_적용() 을 실행하세요.');
  } else {
    Logger.log('\n  ✅ 다 바꿨습니다.');
    Logger.log('  ⚠️ 세금 예수금이 늘어난 만큼 「매출 순 이익」이 줄어듭니다. 정상입니다.');
    Logger.log('  ⚠️ 비율을 다시 맞추려면 코드의 새비율_ 을 고치고 이 함수를 또 돌리면 됩니다.');
  }
}


// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  🧱 수기채우기 1단계 — 시트에 아예 없던 큰 것들  (원당점)
//
//  2026-09-13 추가. 사장님 수기 손익계산서(1~6월·8월)가 근거입니다.
//
//  ── 이번에 넣는 것 ──────────────────────────────────────
//     4대보험     매달 300만~620만. 시트에 한 번도 안 들어갔습니다.
//                 ⚠️ 1~4월 수기는 산재·연금·건강·고용이 따로 적혀 있어 합쳤습니다.
//     택배비      고기 택배비라 「고기값」으로 넣습니다 (사장님 지시).
//     가스사용료   5월부터 시트에 없습니다.
//     매장관리비   전기+수도를 합쳐 넣습니다.
//     배달매출     수기 수입과 시트 매출의 차액. 포스에 안 잡히는 매출입니다.
//
//  ── 일부러 안 넣는 것 ────────────────────────────────────
//     부가세·종합소득세   시트가 월별로 따로 계산합니다. 넣으면 두 번 잡힙니다.
//     퇴직금·미광급여     퇴직금이라 손익에 안 넣습니다 (사장님 확인).
//     인건비             ⚠️ 알바계산기가 기준입니다. 수기와 달라도 건드리지 않습니다.
//     원가·식자재         이미 영수증으로 들어간 것과 겹칩니다. 2단계에서 대조합니다.
//
//  ⚠️ 7월 수기 손익계산서가 없어 7월은 비워뒀습니다.
//  ⚠️ 같은 달·같은 계정에 이미 값이 있으면 건너뜁니다. 두 번 안 들어갑니다.
//  ⚠️ 되돌리려면 [수기1단계] 표식이 붙은 줄을 지우면 됩니다.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

//  월 → { 계정: 금액 }   (0 이거나 없는 항목은 안 넣습니다)
var 수기1단계_원당_ = {
  1: { '4대보험': 4528090, '고기값': 1220000, '가스사용료':  748030, '매장관리비':  764890, '배달매출': 2959870 },
  2: { '4대보험': 3164260, '고기값': 1000000, '가스사용료':  829750, '매장관리비':  768350, '배달매출': 1371940 },
  // ⚠️ 2026-09-13 — 3·4월 택배비는 넣지 않습니다.
  //    고기값복원() 때 「남길 것」으로 안 지운 택배비가 이미 고기값 계정에 있습니다.
  //        3월 시트 고기값 12,825,000 = 11,170,000 + 택배비 1,250,000 + 축산 405,000
  //        4월 시트 고기값 13,930,000 = 12,570,000 + 택배비 1,360,000
  //    여기서 또 넣으면 261만원이 두 번 잡힙니다.
  3: { '4대보험': 3341760,                    '가스사용료':  974320, '매장관리비':  592020, '배달매출': 2314800 },
  4: { '4대보험': 4334800,                    '가스사용료':  771240, '매장관리비':  578970, '배달매출': 2587000 },
  5: { '4대보험': 3775820, '고기값': 1300000, '가스사용료':  373350, '매장관리비':  712500, '배달매출': 3150000 },
  6: { '4대보험': 5502360, '고기값': 1250000, '가스사용료':  767360, '매장관리비':  945290, '배달매출': 1887000 },
  // 7월 — 수기 손익계산서를 아직 못 받았습니다
  8: { '4대보험': 6176850, '고기값': 1305000, '가스사용료': 1117740, '매장관리비': 1309400, '배달매출': 2442000 },
};

var 수기표식_ = '[수기1단계]';

// ⚠️ 2026-09-13 — 계정마다 처리가 다릅니다. 첫 판에서 전부 똑같이 다뤄 틀렸습니다.
//
//   있으면건너뜀   같은 것이 이미 들어간 경우. 또 넣으면 두 배가 됩니다.
//                 예) 4대보험·가스사용료 — 3·4월에 이미 있습니다.
//
//   항상추가       시트에 있는 것과 「다른 항목」이라 더해야 합니다.
//                 예) 택배비 — 고기값 계정에 넣지만 고기값 자체와는 별개 금액입니다.
//                     배달매출 — 차액이 곧 부족분입니다. 이미 있는 것은 이미 센 것입니다.
//
//   차액만         수기 금액에서 시트에 이미 있는 만큼을 빼고 넣습니다.
//                 예) 매장관리비 — 4월에 전기요금만 있고 수도가 빠져 있습니다.
var 처리방식_ = {
  '4대보험'    : '있으면건너뜀',
  '가스사용료'  : '있으면건너뜀',
  '고기값'     : '항상추가',
...
// 항목별 메모 (로그에 같이 찍어 판단을 돕는다)
var 수기메모_ = {
  '고기값'    : '택배비 — 고기 택배비라 고기값으로',
  '매장관리비' : '전기 + 수도',
  '4대보험'   : '산재·연금·건강·고용 합계',
  '배달매출'  : '수기 수입 − 시트 매출',
};

function 수기채우기_미리보기() { 수기채우기_(true); }
function 수기채우기_적용()     { 수기채우기_(false); }

function 수기채우기_(dryRun) {
  var branch = '원당점';
  var sh = SpreadsheetApp.openById(BRANCH_CONFIG[branch].ssId).getSheetByName('지출및매출로그');
  if (!sh) { Logger.log('❌ 시트 없음'); return; }
  ...
  if (sh.getLastRow() > 1) {
    ...
    v.forEach(function (r) {
      var d = toDate_(r[0]);
      if (!d || d.getFullYear() !== 2026) return;
      var key = (d.getMonth() + 1) + '|' + String(r[1]).trim();
      있음[key] = (있음[key] || 0) + (Number(r[3]) || 0);
    });
  }
  ...
  Logger.log('\n ════════ 수기 손익계산서로 채우기 · 1단계 [' + branch + '] ════════\n');

  Object.keys(수기1단계_원당_).sort(function (a, b) { return a - b; }).forEach(function (m) {
    var 말일 = new Date(2026, Number(m), 0);
    var ymd  = Utilities.formatDate(말일, TIMEZONE, 'yyyy-MM-dd');
    var 항목 = 수기1단계_원당_[m];

    Logger.log('  ── ' + m + '월 ──');
    Object.keys(항목).forEach(function (cat) {
      var amt = 항목[cat];
      if (!amt) return;
      var 기존   = 있음[m + '|' + cat] || 0;
      var 방식   = 처리방식_[cat] || '있으면건너뜀';
      var 넣을돈 = amt;
      var 설명   = '';

      if (방식 === '있으면건너뜀' && 기존 > 0) {
        건너뛴것.push(m + '월 ' + cat + ' — 이미 ' + 기존.toLocaleString() + '원 있음');
        Logger.log('     ⏭ ' + (cat + '          ').slice(0, 11) +
                   (amt.toLocaleString() + '원          ').slice(0, 13) +
                   '  이미 ' + 기존.toLocaleString() + '원 있어서 건너뜀');
        return;
      }
      ...
      if (방식 === '차액만') {
        넣을돈 = amt - 기존;
        if (넣을돈 <= 0) {
          건너뛴것.push(m + '월 ' + cat + ' — 이미 충분히 들어감');
          Logger.log('     ⏭ ' + (cat + '          ').slice(0, 11) +
                     (amt.toLocaleString() + '원          ').slice(0, 13) +
                     '  이미 ' + 기존.toLocaleString() + '원 있어 넣을 것 없음');
          return;
        }
        if (기존 > 0) 설명 = '  (수기 ' + amt.toLocaleString() + ' − 시트 ' + 기존.toLocaleString() + ')';
      }

      if (방식 === '항상추가' && 기존 > 0) {
        설명 = '  (시트에 이미 ' + 기존.toLocaleString() + '원 있지만 별개 항목이라 더합니다)';
      }

      var 메모 = 수기메모_[cat] ? ('  ← ' + 수기메모_[cat]) : '';
      Logger.log('     ✅ ' + (cat + '          ').slice(0, 11) +
                 (넣을돈.toLocaleString() + '원          ').slice(0, 13) + 메모 + 설명);

      넣을줄.push([ymd, cat, 수기표식_ + ' ' + m + '월 ' + (수기메모_[cat] || cat),
                   넣을돈, branch, 수기표식_, now, '']);
      합계 += 넣을돈;
    });
  });

  Logger.log('\n  ──────────────────────────────');
  Logger.log('  넣을 것 ' + 넣을줄.length + '줄 · ' + 합계.toLocaleString() + '원');
  if (건너뛴것.length) Logger.log('  건너뛴 것 ' + 건너뛴것.length + '개 (이미 있어서)');
  Logger.log('\n  ⚠️ 7월은 수기 손익계산서를 못 받아 비워뒀습니다.');
  Logger.log('  ⚠️ 배달매출은 「매출」이라 이익이 늘고, 나머지는 「비용」이라 이익이 줄어듭니다.');

  if (dryRun) {
    Logger.log('\n  ※ 미리보기입니다. 실제로 하려면 수기채우기_적용() 을 실행하세요.');
    return;
  }
  if (!넣을줄.length) { Logger.log('\n  넣을 것이 없습니다.'); return; }
  ...
  sh.getRange(sh.getLastRow() + 1, 1, 넣을줄.length, SHEET_HEADERS.length).setValues(넣을줄);
  Logger.log('\n  ✅ ' + 넣을줄.length + '줄 · ' + 합계.toLocaleString() + '원을 넣었습니다.');
  Logger.log('  다음: 팔월대조() 로 8월이 얼마나 맞아졌는지 보세요.');
}


// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  🧱 수기채우기 2단계 — 시트에 자리조차 없던 것들  (원당점)
//
//  2026-09-13 추가. 1단계에 이어서입니다.
//
//  ── 어디로 보내나 (사장님 확정) ──────────────────────────
//     특양                    → 가게외부카드   (네이버·쿠팡·특양 묶음)
//     공동카메라·수리·하수도청소  → 매장관리비     (가게 관리 성격)
//     돌핀·매쉬부룽·설추석휴가비  → 기타잡비용
//     지방세·등록세·면허세       → 기타세금
//
//  ── 일부러 안 넣는 것 ────────────────────────────────────
//     근로소득세·사업소득세   ⚠️ 알바계산기가 「세전(gross)」을 보냅니다.
//                            알바는 3.3%, 직원급여도 세전으로 들어갑니다.
//                            세금이 이미 인건비에 포함돼 있어 또 넣으면 두 배입니다.
//     부가세·종합소득세      시트가 매출 비례로 따로 계산합니다.
//     퇴직소득세·퇴직금·미광급여  퇴직금은 손익에 안 넣기로 했습니다.
//     「없음」 1,570,420원   1월에만 있고 정체를 모릅니다. 알게 되면 넣습니다.
//
//  ⚠️ 원가(주류·음료·미락·가게내부카드)는 3단계입니다.
//     영수증으로 들어간 것과 겹쳐서 하나씩 대조해야 합니다.
// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

var 수기2단계_원당_ = {
  1: { '기타잡비용': 393000, '기타세금': 213820 },   // 돌핀4만+메쉬부룽30만+위생교육비5.3만 / 지방세18.68만+등록세면허세2.7만
  2: { '기타잡비용': 1178000, '기타세금': 0 },       // 돌핀3.3만+메쉬부룽30만+설추석휴가80만+위생교육4.5만
  3: { '기타잡비용': 575000,  '기타세금': 0 },       // 잡비(에어컨청소외)53만+위생교육4.5만
  // ⚠️ 2026-09-13 — 4월 특양은 빼둡니다. 시트 가게외부카드 4월에 이미 249,150원이 있습니다
  //    (특양 249,150 + 현대카드 1,445,400 = 1,694,550). 넣으면 두 번 잡힙니다.
  4: { '기타잡비용': 234000,  '기타세금': 0 },       // 잡비8.3만+돌핀10.6만+위생교육4.5만
  5: { '기타잡비용': 728000,  '기타세금': 0 },       // 잡비64.8만+돌핀3.5만+위생교육4.5만
  // ⚠️ 6월 정수기청소 19,900원도 빼둡니다. 시트 「고정비」에 정수기렌탈 19,900원이 이미 있습니다.
  6: { '기타잡비용': 345000,  '기타세금': 0 },       // 메쉬부룽30만+위생교육4.5만
  // 7월 — 수기 손익계산서 없음
  8: { '기타잡비용': 640000,  '기타세금': 0, '가게외부카드': 303644, '매장관리비': 340000 },
                                                    // 돌핀4만+메쉬부룽30만+설추석휴가30만 / 특양 / 공동카메라·화장실수리·하수도청소
};

var 수기2표식_ = '[수기2단계]';

var 수기2메모_ = {
  '기타잡비용'  : '돌핀·매쉬부룽·설추석휴가·위생교육·잡비',
  '기타세금'   : '지방세·등록세·면허세',
  '가게외부카드' : '특양',
  '매장관리비'  : '공동카메라·수리·정수기청소',
};

function 수기2단계_미리보기() { 수기2단계_(true); }
function 수기2단계_적용()     { 수기2단계_(false); }

function 수기2단계_(dryRun) {
  var branch = '원당점';
  var sh = SpreadsheetApp.openById(BRANCH_CONFIG[branch].ssId).getSheetByName('지출및매출로그');
  if (!sh) { Logger.log('❌ 시트 없음'); return; }
  ...
  if (sh.getLastRow() > 1) {
    ...
    v.forEach(function (r) {
      var d = toDate_(r[0]);
      if (!d || d.getFullYear() !== 2026) return;
      있음[(d.getMonth() + 1) + '|' + String(r[1]).trim()] =
        (있음[(d.getMonth() + 1) + '|' + String(r[1]).trim()] || 0) + (Number(r[3]) || 0);
    });
  }
  ...
  Logger.log('\n ════════ 수기 손익계산서로 채우기 · 2단계 [' + branch + '] ════════\n');

  Object.keys(수기2단계_원당_).sort(function (a, b) { return a - b; }).forEach(function (m) {
    var 말일 = new Date(2026, Number(m), 0);
    var ymd  = Utilities.formatDate(말일, TIMEZONE, 'yyyy-MM-dd');
    var 항목 = 수기2단계_원당_[m];
    var 찍음 = false;
    ...
    Object.keys(항목).forEach(function (cat) {
      var amt = 항목[cat];
      if (!amt) return;
      if (!찍음) { Logger.log('  ── ' + m + '월 ──'); 찍음 = true; }

      var 기존 = 있음[m + '|' + cat] || 0;
      var 메모 = 수기2메모_[cat] ? ('  ← ' + 수기2메모_[cat]) : '';
      var 설명 = 기존 > 0 ? ('  (시트에 이미 ' + 기존.toLocaleString() + '원 있지만 별개 항목이라 더합니다)') : '';

      Logger.log('     ✅ ' + (cat + '            ').slice(0, 13) +
                 (amt.toLocaleString() + '원          ').slice(0, 13) + 메모 + 설명);

      넣을줄.push([ymd, cat, 수기2표식_ + ' ' + m + '월 ' + (수기2메모_[cat] || cat),
                   amt, branch, 수기2표식_, now, '']);
      합계 += amt;
    });
  });
  ...
  Logger.log('\n  ──────────────────────────────');
  Logger.log('  넣을 것 ' + 넣을줄.length + '줄 · ' + 합계.toLocaleString() + '원');
  Logger.log('\n  ⚠️ 전부 「비용」입니다. 그만큼 이익이 줄어듭니다.');
  Logger.log('  ⚠️ 7월은 수기 손익계산서를 못 받아 비워뒀습니다.');

  if (dryRun) {
    Logger.log('\n  ※ 미리보기입니다. 실제로 하려면 수기2단계_적용() 을 실행하세요.');
    return;
  }
  if (!넣을줄.length) { Logger.log('\n  넣을 것이 없습니다.'); return; }
  ...
  sh.getRange(sh.getLastRow() + 1, 1, 넣을줄.length, SHEET_HEADERS.length).setValues(넣을줄);
  Logger.log('\n  ✅ ' + 넣을줄.length + '줄 · ' + 합계.toLocaleString() + '원을 넣었습니다.');
  Logger.log('  다음: 팔월대조() 로 8월에 뭐가 남았는지 보세요.');
}


// ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
//  🔍 계정내역() — 계정 하나를 골라 그 안에 뭐가 들었는지 전부 본다
//
//  2026-09-13 추가.
//  원당 「기타잡비용」에 4월 783만·5월 1,090만이 들어 있는데, 수기 손익계산서에
//  그만한 잡비가 없습니다. 부가세·퇴직금·종합소득세가 잘못 분류된 것으로 보입니다.
//  그 안을 눈으로 봐야 빼낼 수 있습니다.
//
...
function 계정내역() {
  ...
  v.forEach(function (r) {
    if (String(r[1]).trim() !== 볼계정_) return;
    var d = toDate_(r[0]);
    if (!d || d.getFullYear() !== 2026) return;
    var m = d.getMonth() + 1;
    if (!월별[m]) 월별[m] = [];
    월별[m].push({
      ymd : Utilities.formatDate(d, TIMEZONE, 'MM-dd'),
      금액 : Number(r[3]) || 0,
      항목 : String(r[2] || '').trim(),
      표식 : String(r[5] || '').trim(),
    });
  ...
  Logger.log('\n ════════ [' + 볼지점_ + '] 「' + 볼계정_ + '」 전체 내역 ════════');
  Logger.log('  ' + 줄수 + '줄 · ' + 총합.toLocaleString() + '원\n');

  Object.keys(월별).sort(function (a, b) { return a - b; }).forEach(function (m) {
    var 줄 = 월별[m];
    var 월합 = 0;
    줄.forEach(function (x) { 월합 += x.금액; });
